...
@section('title', 'Regimes Alimentaires')
...
<div class="ml-64 p-6 min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
            <h1 class="text-3xl font-bold text-blue-800">Créer un Nouveau Régime Alimentaire</h1>
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden p-4">
            <form action="{{ route('regimes.store') }}" method="POST" class="p-8">
                @csrf
                ...
                <div class="space-y-6">
                            ...
                            <select name="statut_code" id="statut_code" 
                                ...
                                @foreach ($regimeStatuses as $status)
                                    <option value="{{ $status->code }}" 
                                        {{ (old('statut_code', $defaultStatus ?? 'ACT') == $status->code ? 'selected' : '') }}>
                                        {{ $status->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="calories_journalieres" id="calories_journalieres"
                            value="{{ old('calories_journalieres') }}"
                    ...
                    <div>
                        ...
                        @php
                            ...
                            if (is_array(old('restrictions'))) {
                                ...
                                $restrictionsValue = old('restrictions', '');
                            }
                        @endphp
                    ...
                    <div>
                        ...
                        <textarea name="recommandations" id="recommandations" rows="4"
                            ...
                            placeholder="Ajouter des recommandations spécifiques pour le patient...">{{ old('recommandations') }}</textarea>
